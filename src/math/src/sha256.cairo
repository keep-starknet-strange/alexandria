...
#[inline]
fn sha256<T, +Drop<T>, +ByteReader<T>, +IndexView<ByteReaderState<T>, usize, @u8>>(
    data: T
) -> Array<u8> {
...
#[generate_trait]
impl Sha256Impl<
    T, +Drop<T>, +ByteReader<T>, +IndexView<ByteReaderState<T>, usize, @u8>
> of Sha256Trait<T> {
    #[inline]
...
> of Sha256ByteReaderTrait<T> {
    ...
    fn sha256_word(ref self: ByteReaderState<T>) -> u32 {
        let result: u32 = (*self[self.index]).into() * 0x1000000
            + (*self[self.index + 1]).into() * 0x10000
            + (*self[self.index + 2]).into() * 0x100
            + (*self[self.index + 3]).into();
        self.index += 4;
        result
    }
