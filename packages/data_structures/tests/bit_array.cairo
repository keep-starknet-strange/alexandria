...
fn test_read_word_be() {
    let mut ba = sample_bit_array();
    assert!(ba.read_word_be(length: 128).unwrap() == Bounded::<u128>::MAX.into() - 1);
}
...
fn test_write_word_be() {
    ...
    assert!(ba.read_word_be(128).unwrap() == Bounded::<u128>::MAX.into() - 2);
}
...
fn test_write_word_le() {
    ...
    assert!(ba.read_word_le(128).unwrap() == Bounded::<u128>::MAX.into() - 4);
}
