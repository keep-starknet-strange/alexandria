// Internal imports
use alexandria_data_structures::vec::{Felt252Vec, NullableVec, VecTrait};
use core::ops::index::Index;

fn vec_new_test<V, T, +VecTrait<V, T>>(vec: @V) {
    assert(vec.len() == 0, 'values should match');
}

fn vec_len_test<V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>>(
    ref vec: V, val_1: T,
) {
    vec.push(val_1);
    assert(vec.len() == 1, 'values should match');
}

fn vec_get_test<V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>>(
    ref vec: V, val_1: T,
) {
    vec.push(val_1);
    assert(vec.get(0).unwrap() == val_1, 'values should match');
    assert(vec.get(1).is_none(), 'should be none');
}

fn vec_at_test<V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>>(
    ref vec: V, val_1: T,
) {
    vec.push(val_1);
    assert(vec.at(0) == val_1, 'values should match');
}

fn vec_at_out_of_bounds_test<
    V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>,
>(
    ref vec: V,
) {
    vec.at(0);
}

fn vec_push_test<V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>>(
    ref vec: V, val_1: T,
) {
    vec.push(val_1);
    assert(vec.len() == 1, 'values should match');
    assert(vec.at(0) == val_1, 'values should match');
}

fn vec_set_test<V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>>(
    ref vec: V, val_1: T, val_2: T,
) {
    vec.push(val_1);
    vec.set(0, val_2);
    let result = vec.get(0);
    assert(result.unwrap() == val_2, 'values should match');
}

fn vec_set_test_expect_error<
    V, T, +VecTrait<V, T>, +Drop<T>, +Copy<T>, +PartialEq<T>, +Destruct<V>,
>(
    ref vec: V, val_1: T, val_2: T,
) {
    vec.push(val_1);
    vec.set(1, val_2);
}

fn vec_index_trait_test<
    V,
    T,
    +VecTrait<V, T>,
    impl IndexImpl: Index<V, usize>,
    +Drop<T>,
    +Copy<T>,
    +PartialEq<T>,
    +Destruct<V>,
    +Into<IndexImpl::Target, T>,
>(
    ref vec: V, val_1: T, val_2: T,
) {
    vec.push(val_1);
    vec.push(val_2);
    assert(vec[0].into() == val_1, 'values should match');
    assert(vec[1].into() == val_2, 'values should match');
}

fn vec_index_trait_out_of_bounds_test<
    V,
    T,
    +VecTrait<V, T>,
    impl IndexImpl: Index<V, usize>,
    +Drop<T>,
    +Copy<T>,
    +Drop<IndexImpl::Target>,
    +Copy<IndexImpl::Target>,
    +PartialEq<T>,
    +Destruct<V>,
>(
    ref vec: V, val_1: T,
) {
    vec[0];
}


#[test]
fn felt252_vec_new_test() {
    vec_new_test(@VecTrait::<Felt252Vec, u128>::new());
}

#[test]
fn felt252_vec_len_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_len_test(ref vec, 1);
}

#[test]
fn felt252_vec_get_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_get_test(ref vec, 1);
}

#[test]
fn felt252_vec_at_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_at_test(ref vec, 1);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn felt252_vec_at_out_of_bounds_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_at_out_of_bounds_test(ref vec);
}

#[test]
fn felt252_vec_push_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_push_test(ref vec, 1);
}

#[test]
fn felt252_vec_set_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_set_test(ref vec, 1, 2);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn felt252_vec_set_test_expect_error() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_set_test_expect_error(ref vec, 1, 2);
}

#[test]
fn felt252_vec_index_trait_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_index_trait_test(ref vec, 1, 2);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn felt252_vec_index_trait_out_of_bounds_test() {
    let mut vec = VecTrait::<Felt252Vec, u128>::new();
    vec_index_trait_out_of_bounds_test(ref vec, 1);
}

#[test]
fn nullable_vec_new_test() {
    vec_new_test(@VecTrait::<NullableVec, u128>::new());
}

#[test]
fn nullable_vec_len_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_len_test(ref vec, 1);
}

#[test]
fn nullable_vec_get_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_get_test(ref vec, 1);
}

#[test]
fn nullable_vec_at_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_at_test(ref vec, 1);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn nullable_vec_at_out_of_bounds_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_at_out_of_bounds_test(ref vec);
}

#[test]
fn nullable_vec_push_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_push_test(ref vec, 1);
}

#[test]
fn nullable_vec_set_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_set_test(ref vec, 1, 2);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn nullable_vec_set_test_expect_error() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_set_test_expect_error(ref vec, 1, 2);
}

#[test]
fn nullable_vec_index_trait_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_index_trait_test(ref vec, 1, 2);
}

#[test]
#[should_panic(expected: ('Index out of bounds',))]
fn nullable_vec_index_trait_out_of_bounds_test() {
    let mut vec = VecTrait::<NullableVec, u128>::new();
    vec_index_trait_out_of_bounds_test(ref vec, 1);
}
