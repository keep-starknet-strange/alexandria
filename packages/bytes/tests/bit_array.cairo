...
fn test_append_bit() {
    ...
    assert!(ba.current() == 0xa * one_shift_left_bytes_felt252(30) * shift_bit(4).into());
...
fn test_read_word_be() {
    let mut ba = sample_bit_array();
    assert!(
        ba.read_word_be(length: 128).unwrap() == Bounded::<u128>::MAX.into() - 1,
        "read_word_be mismatch",
    );
}
...
fn test_read_word_le() {
    ...
    assert!(ba.read_word_le(length: 128).unwrap() == value.into(), "value mismatch");
}
...
fn test_read_word_be_u256() {
    ...
    assert!(
        ba.read_word_be_u256(length: 256).unwrap() == u256 { low, high },
        "read_word_be_u256 mismatch",
    );
}
...
fn test_read_word_le_u256() {
    ...
    assert!(
        ba.read_word_le_u256(length: 256).unwrap() == u256 { low, high },
        "read_word_le_u256 mismatch",
    );
}
...
fn test_write_word_be() {
    ...
    assert!(
        ba.read_word_be(128).unwrap() == Bounded::<u128>::MAX.into() - 2, "read_word_be mismatch",
    );
}
...
fn test_write_word_le() {
    ...
    assert!(
        ba.read_word_le(128).unwrap() == Bounded::<u128>::MAX.into() - 4, "read_word_le mismatch",
    );
}
...
// helpers
// Creates a BitArray with pre-filled data: (u128::MAX - 1) * 256^15
// NOTE: This is suitable for BE tests and basic BitArray tests, but should NOT
// be used for LE tests. For LE tests, create a fresh BitArray and use write_word_le()
// to ensure the data is in the correct byte-order little-endian format.
