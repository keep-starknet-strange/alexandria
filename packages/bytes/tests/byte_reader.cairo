use alexandria_bytes::byte_reader::{ByteReader, ByteReaderState};
...
fn test_reader_helper() {
    ...
    // reader.data member is not visible in this context
    let first_byte = reader.read_u8().unwrap();
    assert(first_byte == 1, 'first byte should be 1');
}
...
fn test_read_sequence() {
    ...
    assert!(
        rd.read_i128() == Option::Some(0x02030405060708090a0b0c0d0e0f1011), "read_i128 mismatch",
    );
    assert!(
        rd.read_u128() == Option::Some(0x12131415161718191a1b1c1d1e1f2021), "read_u128 mismatch",
    );
    assert!(rd.read_i64() == Option::Some(0x2223242526272829));
    assert!(
        rd.read_u128() == Option::Some(0x2a2b2c2d2e2f30313233343536373839), "read_u128 mismatch",
    );
...
fn test_read_sequence_le() {
    ...
    assert!(rd.read_i8() == Option::Some(1), "expected 1");
    assert!(
        rd.read_i128_le() == Option::Some(0x11100f0e0d0c0b0a0908070605040302),
        "read_i128_le mismatch",
    );
    assert!(
        rd.read_u128_le() == Option::Some(0x21201f1e1d1c1b1a1918171615141312),
        "read_u128_le mismatch",
    );
...
fn serialized_byte_array_64() -> Array<felt252> {
    array![
        0x40, 0x0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f,
        0x202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e, 0x3f40,
    ]
}
