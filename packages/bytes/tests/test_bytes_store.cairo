...
mod tests {
    ...
    fn deploy() -> IABytesStoreDispatcher {
        let contract = declare("ABytesStore").unwrap().contract_class();
        let (contract_address, _) = contract.deploy(@ArrayTrait::new()).unwrap();
        IABytesStoreDispatcher { contract_address }
    }

    #[test]
    fn test_deploy() {
        let contract = deploy();
        assert!(contract.get_bytes() == BytesTrait::new_empty());
    }
    ...
    fn test_bytes_storage_32_bytes() {
        ...
        assert!(contract.get_bytes() == bytes);
    }
