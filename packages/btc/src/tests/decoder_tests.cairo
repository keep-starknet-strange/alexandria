...
fn test_u64_le_reading() {
    ...
    assert_eq!(result, 0x1122334455667788);
}
...
fn test_legacy_transaction_decoding() {
    ...
    assert_eq!(output.script_pubkey.len(), 25);
}
...
fn test_segwit_transaction_detection() {
    ...
    assert!(tx.is_segwit);
    assert_eq!(tx.witness.len(), tx.inputs.len());
}
...
fn test_witness_data_decoding() {
    ...
    assert_eq!(witness.witness_stack.len(), 2);
    assert_eq!(witness.witness_stack.at(0).len(), 71);
    assert_eq!(witness.witness_stack.at(1).len(), 33);
}
...
fn test_multi_input_output_transaction() {
    ...
    assert_eq!(*tx.outputs.at(0).value, 1);
    // Check second output value
    assert_eq!(*tx.outputs.at(1).value, 2);
}
...
fn test_real_bitcoin_transaction_decoding() {
    ...
    assert_eq!(*input1.previous_vout, 1);
    assert_eq!(*input1.sequence, 0xffffffff);
    assert_eq!(input1.script_sig.len(), 138);
    assert_eq!(input1.previous_txid.len(), 32);

    // Verify first input script signature content (first few bytes)
    assert_eq!(input1.script_sig.at(0).unwrap(), 0x47); // DER signature length
    assert_eq!(input1.script_sig.at(1).unwrap(), 0x30); // DER sequence tag
    assert_eq!(input1.script_sig.at(2).unwrap(), 0x44); // DER sequence length
    assert_eq!(input1.script_sig.at(3).unwrap(), 0x02); // DER integer tag (r value)
    assert_eq!(input1.script_sig.at(4).unwrap(), 0x20); // r value length (32 bytes)

    // Verify second input
    let input2 = tx.inputs.at(1);
    assert_eq!(*input2.previous_vout, 1);
    assert_eq!(*input2.sequence, 0xffffffff);
    assert_eq!(input2.script_sig.len(), 138);
    assert_eq!(input2.previous_txid.len(), 32);

    // Verify second input script signature content (first few bytes)
    assert_eq!(input2.script_sig.at(0).unwrap(), 0x47); // DER signature length
    assert_eq!(input2.script_sig.at(1).unwrap(), 0x30); // DER sequence tag
    assert_eq!(input2.script_sig.at(2).unwrap(), 0x44); // DER sequence length
    assert_eq!(input2.script_sig.at(3).unwrap(), 0x02); // DER integer tag (r value)
    assert_eq!(input2.script_sig.at(4).unwrap(), 0x20); // r value length (32 bytes)
    ...
    assert_eq!(input1.script_sig.at(137).unwrap(), 0x7a); // Last byte of public key
    assert_eq!(input2.script_sig.at(137).unwrap(), 0x7a); // Last byte of public key
    assert_eq!(input1.script_sig.at(136).unwrap(), 0x04); // Second to last byte
    assert_eq!(input2.script_sig.at(136).unwrap(), 0x04); // Second to last byte

    // Verify first output (1420100 satoshis)
    let output1 = tx.outputs.at(0);
    assert_eq!(*output1.value, 1420100);
    assert_eq!(output1.script_pubkey.len(), 25);

    // Verify second output (69239 satoshis)
    let output2 = tx.outputs.at(1);
    assert_eq!(*output2.value, 69239);
    assert_eq!(output2.script_pubkey.len(), 25);
}
...
fn test_maximum_values() {
    ...
    assert_eq!(tx.version, 0xffffffff);
    assert_eq!(tx.inputs.len(), 1);
    assert_eq!(*tx.inputs.at(0).previous_vout, 0xffffffff);
    assert_eq!(*tx.inputs.at(0).sequence, 0xffffffff);
    assert_eq!(tx.outputs.len(), 1);
    assert_eq!(*tx.outputs.at(0).value, 0xffffffffffffffff);
    assert_eq!(tx.locktime, 0xffffffff);
}
...
fn test_large_script_sizes() {
    ...
    assert_eq!(tx.inputs.at(0).script_sig.len(), 252);
    assert_eq!(tx.outputs.at(0).script_pubkey.len(), 252);

    // Verify script content
    assert_eq!(tx.inputs.at(0).script_sig.at(0).unwrap(), 0x6a);
    assert_eq!(tx.inputs.at(0).script_sig.at(251).unwrap(), 0x6a);
    assert_eq!(tx.outputs.at(0).script_pubkey.at(0).unwrap(), 0x51);
    assert_eq!(tx.outputs.at(0).script_pubkey.at(251).unwrap(), 0x51);
}
