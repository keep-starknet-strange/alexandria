...
fn test_compact_size_encoding() {
    ...
    assert_eq!(encoder.data.at(0).unwrap(), 0x42);
    assert_eq!(encoder.data.len(), 1);

    // Test 3-byte compact size (0xfd + 2 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x1234);
    assert_eq!(encoder.data.len(), 3); // Total length should be 3
    assert_eq!(encoder.data.at(0).unwrap(), 0xfd);
    // Check little-endian encoding of 0x1234 (2 bytes)
    assert_eq!(encoder.data.at(1).unwrap(), 0x34);
    assert_eq!(encoder.data.at(2).unwrap(), 0x12);

    // Test 5-byte compact size (0xfe + 4 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x12345678);
    assert_eq!(encoder.data.len(), 5); // Total length should be 5
    assert_eq!(encoder.data.at(0).unwrap(), 0xfe);
    // Check little-endian encoding of 0x12345678 (4 bytes)
    assert_eq!(encoder.data.at(1).unwrap(), 0x78);
    assert_eq!(encoder.data.at(2).unwrap(), 0x56);
    assert_eq!(encoder.data.at(3).unwrap(), 0x34);
    assert_eq!(encoder.data.at(4).unwrap(), 0x12);

    // Test 9-byte compact size (0xff + 8 bytes)
    let mut encoder = TransactionEncoderTrait::new();
    encoder.write_compact_size(0x1122334455667788);
    assert_eq!(encoder.data.len(), 9); // Total length should be 9
    assert_eq!(encoder.data.at(0).unwrap(), 0xff);
    // Check little-endian encoding of 0x1122334455667788 (8 bytes)
    assert_eq!(encoder.data.at(1).unwrap(), 0x88);
    assert_eq!(encoder.data.at(2).unwrap(), 0x77);
    assert_eq!(encoder.data.at(3).unwrap(), 0x66);
    assert_eq!(encoder.data.at(4).unwrap(), 0x55);
    assert_eq!(encoder.data.at(5).unwrap(), 0x44);
    assert_eq!(encoder.data.at(6).unwrap(), 0x33);
    assert_eq!(encoder.data.at(7).unwrap(), 0x22);
    assert_eq!(encoder.data.at(8).unwrap(), 0x11);
}
...
fn test_u32_le_encoding() {
    ...
    assert_eq!(encoder.data.at(0).unwrap(), 0x78);
    assert_eq!(encoder.data.at(1).unwrap(), 0x56);
    assert_eq!(encoder.data.at(2).unwrap(), 0x34);
    assert_eq!(encoder.data.at(3).unwrap(), 0x12);
}
...
fn test_u64_le_encoding() {
    ...
    // Verify little-endian encoding
    assert_eq!(encoder.data.at(0).unwrap(), 0x88);
    assert_eq!(encoder.data.at(1).unwrap(), 0x77);
    assert_eq!(encoder.data.at(2).unwrap(), 0x66);
    assert_eq!(encoder.data.at(3).unwrap(), 0x55);
    assert_eq!(encoder.data.at(4).unwrap(), 0x44);
    assert_eq!(encoder.data.at(5).unwrap(), 0x33);
    assert_eq!(encoder.data.at(6).unwrap(), 0x22);
    assert_eq!(encoder.data.at(7).unwrap(), 0x11);
}
...
fn test_simple_legacy_transaction_encoding() {
    ...
    assert_eq!(decoded_tx.locktime, 0);
    assert!(!decoded_tx.is_segwit);

    // Check input
    let decoded_input = decoded_tx.inputs.at(0);
    assert_eq!(*decoded_input.previous_vout, 0);
    assert_eq!(*decoded_input.sequence, 0xffffffff);
    assert_eq!(decoded_input.script_sig.len(), 0);
    assert_eq!(decoded_input.previous_txid.len(), 32);

    // Check output
    let decoded_output = decoded_tx.outputs.at(0);
    assert_eq!(*decoded_output.value, 1);
    assert_eq!(decoded_output.script_pubkey.len(), 25);
}
...
fn test_segwit_transaction_encoding() {
    ...
    assert_eq!(encoded_data.at(4).unwrap(), 0x00); // Marker after version
    assert_eq!(encoded_data.at(5).unwrap(), 0x01); // Flag
    ...
    assert_eq!(decoded_tx.locktime, 0);
    assert!(decoded_tx.is_segwit);
    assert_eq!(decoded_tx.witness.len(), 1);

    // Check witness data
    let decoded_witness = decoded_tx.witness.at(0);
    assert_eq!(decoded_witness.witness_stack.len(), 1);
    assert_eq!(decoded_witness.witness_stack.at(0).len(), 71);
}
...
fn test_multi_input_output_transaction_encoding() {
    ...
    assert_eq!(decoded_tx.version, 1);
    assert_eq!(decoded_tx.inputs.len(), 2);
    assert_eq!(decoded_tx.outputs.len(), 2);
    assert_eq!(decoded_tx.locktime, 0);
    assert!(!decoded_tx.is_segwit);
    ...
    assert_eq!(*decoded_input1.previous_vout, 0);
    assert_eq!(*decoded_input2.previous_vout, 1);

    // Check outputs
    let decoded_output1 = decoded_tx.outputs.at(0);
    let decoded_output2 = decoded_tx.outputs.at(1);
    assert_eq!(*decoded_output1.value, 1000);
    assert_eq!(*decoded_output2.value, 2000);
}
...
fn test_round_trip_encoding_decoding() {
    ...
    assert_eq!(decoder.read_compact_size(), 252);
    assert_eq!(decoder.read_compact_size(), 65535);
    assert_eq!(decoder.read_compact_size(), 4294967295);
}
...
fn test_bytes_encoding() {
    ...
    assert_eq!(encoder.data.len(), 4);
    assert_eq!(encoder.data.at(0).unwrap(), 0x01);
    assert_eq!(encoder.data.at(1).unwrap(), 0x02);
    assert_eq!(encoder.data.at(2).unwrap(), 0x03);
    assert_eq!(encoder.data.at(3).unwrap(), 0x04);
}
