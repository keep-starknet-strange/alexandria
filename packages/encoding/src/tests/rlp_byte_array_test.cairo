...
use alexandria_encoding::rlp::{RLPError, RLPItem, RLPItemByteArray, RLPTrait, RLPType};
...
fn test_ba_rlp_encode_empty_input_should_fail() {
    let res = RLPTrait::encode_byte_array(array![].span());
    assert!(res.is_err(), "Should have failed");
    assert!(res.unwrap_err() == RLPError::EmptyInput, "err != EmptyInput");
}
...
fn test_ba_rlp_encode_default_value() {
    ...
    let res = RLPTrait::encode_byte_array(array![input].span()).unwrap();
...
fn test_ba_rlp_encode_mutilple_string() {
    ...
    let res = RLPTrait::encode_byte_array(array![input1, input2].span()).unwrap();
...
fn test_ba_rlp_encode_short_nested_list() {
    ...
    let res = RLPTrait::encode_byte_array(array![list].span()).unwrap();
