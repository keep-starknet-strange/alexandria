...
fn deploy_mock() -> IAListHolderDispatcher {
    let contract = declare("AListHolder").unwrap().contract_class();
    let (contract_address, _) = contract.deploy(@ArrayTrait::new()).unwrap();
    IAListHolderDispatcher { contract_address }
}

fn mock_addr() -> ContractAddress {
    contract_address_const::<'hello'>()
}

#[test]
fn test_deploy() {
    let contract = deploy_mock();
    assert!(contract.do_get_len() == (0, 0));
}
...
fn test_new_initializes_empty_list() {
    ...
    assert!(addresses_list.address_domain == 0);
    assert!(addresses_list.len() == 0);
    assert!(addresses_list.base.into() == addresses_address);
    assert!(addresses_list.storage_size() == 1);
    ...
    assert!(numbers_list.address_domain == 0);
    assert!(numbers_list.len() == 0);
    assert!(numbers_list.base.into() == numbers_address);
    assert!(numbers_list.storage_size() == 2);

    // Check if both addresses and numbers lists are initialized to be empty
    assert!(contract_state.do_get_len() == (0, 0));
    assert!(contract_state.do_is_empty() == (true, true));
}
...
fn test_new_then_fill_list() {
    ...
    assert!(addresses_list.len() == 1);
    assert!(numbers_list.len() == 2);

    assert!(contract_state.do_get_len() == (1, 2));
    assert!(contract_state.do_is_empty() == (false, false));
}
...
fn test_fetch_empty_list() {
    ...
    assert!(empty_list.address_domain == 0);
    assert!(empty_list.len() == 0);
    assert!(empty_list.base.into() == storage_address);
    assert!(empty_list.storage_size() == 1);
}
...
fn test_fetch_existing_list() {
    ...
    assert!(contract_state.do_append(mock_addr, 10) == (0, 0));
    assert!(contract_state.do_append(mock_addr, 20) == (1, 1));

    let addresses_address = contract_state.addresses.as_ptr().__storage_pointer_address__;
    let addresses_list = ListTrait::<ContractAddress>::fetch(0, addresses_address)
        .expect('List fetch failed');
    assert!(addresses_list.address_domain == 0);
    assert!(addresses_list.len() == 2);
    assert!(addresses_list.base.into() == addresses_address);
    assert!(addresses_list.storage_size() == 1);
...
fn test_is_empty() {
    ...
    assert!(contract.do_is_empty() == (true, true));
    contract.do_append(mock_addr(), 1337);
    assert!(contract.do_is_empty() == (false, false));
}
...
fn test_append_few() {
    ...
    assert!(contract.do_append(mock_addr(), 10) == (0, 0));
    assert!(contract.do_append(mock_addr(), 20) == (1, 1));
    assert!(contract.do_get_len() == (2, 2));
}
...
fn test_append_get_many() {
    ...
    while (index != max) {
        ...
        assert!(append_indexes == (index, index));
        index += 1;
    }

    assert!(contract.do_get_len() == (max, max));
    ...
    while (index != max) {
        ...
        assert!(some_addr.unwrap() == mock_addr);
        assert!(some_number.is_some(), "number is some");
        assert!(some_number.unwrap() == index.into());
...
fn test_get_index_pass() {
    ...
    assert!(contract.do_get_index(0) == (mock_addr, 100));
    assert!(contract.do_get_index(1) == (mock_addr, 200));
    assert!(contract.do_get_index(2) == (mock_addr, 300));
}
...
fn test_set_pass() {
    let contract = deploy_mock();
    let mock_addr = mock_addr();
    let diff_addr = contract_address_const::<'bye'>();
    ...
    assert!(contract.do_get_index(0) == (diff_addr, 100));
    assert!(contract.do_get_index(1) == (mock_addr, 20));
    assert!(contract.do_get_index(2) == (diff_addr, 300));
    assert!(contract.do_get_len() == (3, 3));
}
...
fn test_pop_front_pass() {
    ...
    assert!(pop_addr.unwrap() == mock_addr);
    assert!(pop_number.is_some(), "pop number 2 is some");
    assert!(pop_number.unwrap() == 300);
    assert!(contract.do_get_len() == (2, 2));
...
fn test_array_pass() {
    ...
    assert!((array_addr.len(), array_number.len()) == contract.do_get_len());
    assert!((*array_addr[0], *array_number[0]) == contract.do_get_index(0));
    assert!((*array_addr[1], *array_number[1]) == contract.do_get_index(1));
    assert!((*array_addr[2], *array_number[2]) == contract.do_get_index(2));
}
...
fn test_array_empty() {
    ...
    let (array_addr, array_number) = contract.do_array();
    assert!((array_addr.len(), array_number.len()) == (0, 0));
}
...
fn test_array_get_value_after_clean() {
    ...
    assert!(addr.unwrap() == mock_addr);
    assert!(number.is_some(), "number is some");
    assert!(number.unwrap() == 100);
...
fn test_append_span_existing_list() {
    ...
    assert!(contract.do_append(mock_addr, 20) == (1, 1));
    assert!(contract.do_get_len() == (2, 2));
    assert!(contract.do_get_index(0) == (mock_addr, 10));
    assert!(contract.do_get_index(1) == (mock_addr, 20));
    ...
    assert!(contract.do_get_index(0) == (mock_addr, 10));
    assert!(contract.do_get_index(1) == (mock_addr, 20));
    assert!(contract.do_get_index(2) == (mock_addr, 30));
}
