...
mod regular_call_merkle_tree_pedersen {
    ...
    fn regular_call_merkle_tree_pedersen_test() {
        ...
        assert(!result, 'invalid proof failed');
...
fn merkle_tree_pedersen_test() {
    ...
    assert!(computed_proof == valid_proof);
...
fn stored_merkle_tree_pedersen_test() {
    ...
    assert!(computed_proof == expected_proof);
...
fn stored_merkle_tree_efficiency_comparison_test() {
    ...
    for index in test_indices {
        ...
        assert!(regular_proof == stored_proof, "Proofs should match");
        ...
        assert!(regular_root == stored_root, "Roots should match");
...
fn stored_merkle_tree_multi_proof_scenario_test() {
    ...
    for claimer in claimers {
        ...
        assert!(computed_root == merkle_root, "Proof should verify against published root");
    }

    // Verify we generated exactly 4 proofs
    assert!(all_proofs.len() == 4, "Should have generated 4 proofs");
